...
void load_file(const char *f) {
    ...
    lines = 0;
    if (fp) {
        while (fgets(buf[lines], MAX_LINE, fp) && lines < MAX_LINES) {
            buf[lines][strcspn(buf[lines], "\n")] = 0;
            lines++;
        }
        fclose(fp);
    }
    if (lines==0){ buf[0][0]='\0'; lines=1; }
...
static void insert_newline(void) {
    ...
    char right[MAX_LINE];
    strncpy(right, &buf[cy][cx], MAX_LINE - 1);
    right[MAX_LINE - 1] = '\0';
    buf[cy][cx] = '\0';
    for (int i = lines; i > cy + 1; i--) {
        strncpy(buf[i], buf[i - 1], MAX_LINE - 1);
        buf[i][MAX_LINE - 1] = '\0';
    }
    strncpy(buf[cy + 1], right, MAX_LINE - 1);
    buf[cy + 1][MAX_LINE - 1] = '\0';
    lines++;
    cy++;
    cx = 0;
    is_dirty = 1;
...
void draw_status(const char *msg){
    ...
    if(show_status_bar) {
        ...
        snprintf(info, sizeof(info), "%s  Ln %d/%d  Col %d",
                 name, cy + 1, lines, cx + 1);
